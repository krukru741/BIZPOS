            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} onClick={() => setSelected(c)}>
                  ...
                  <td style={{ fontWeight: 700, color: "var(--primary)" }}>₱{c.totalPurchases.toLocaleString("en-PH")}</td>
                  <td style={{ fontSize: 13 }}>{c.totalOrders}</td>
                  <td>
                    {c.outstanding > 0
                      ? <span className="badge badge-red">₱{c.outstanding.toLocaleString("en-PH")}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }} className="animate-slideIn">
            <div className="card" style={{ padding: 20 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, marginBottom: 16 }}>
                ...
                {selected.outstanding > 0 && <span className="badge badge-red">Outstanding: ₱{selected.outstanding.toLocaleString("en-PH")}</span>}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                { label: "Total Spent", value: `₱${selected.totalPurchases.toLocaleString("en-PH")}`, color: "#22C55E" },
            ...
            <div className="card" style={{ padding: 16 }}>
              ...
              {[
                ...
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < 2 ? "1px solid var(--border)" : "none" }}>
                  ...
                  <span style={{ fontWeight: 700, color: "var(--primary)" }}>₱{p.amount.toLocaleString("en-PH")}</span>
                </div>
              ))}
            </div>
          </div>
